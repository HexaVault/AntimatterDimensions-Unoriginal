...
class InfinityDimensionState extends DimensionState {
  constructor(tier) {
    ...
    const COST_MULTS = [null, 1e3, 1e6, 1e8, 1e10, 1e15, 1e20, 1e25, 1e30, 1e35];
    this._costMultiplier = COST_MULTS[tier];
    const POWER_MULTS = [null, 50, 30, 10, 5, 5, 5, 5, 5, 5];
    this._powerMultiplier = POWER_MULTS[tier];
    const BASE_COSTS = [null, 1e8, 1e9, 1e10, 1e20, 1e100, 1e160, 1e210, 1e230, 1e270];
  ...
  get powerMultiplier() {
    return new Decimal(this._powerMultiplier)
      .timesEffectsOf(this._tier === 9 ? GlyphSacrifice.infinity : null)
  ...
  get purchaseCap() {
    ...
    return InfinityDimensions.capIncrease + (this.tier === 9
      ? Number.MAX_VALUE
...
export const InfinityDimensions = {
  ...
  unlockNext() {
    if (InfinityDimension(9).isUnlocked) return;
    this.next().unlock();
  },

  next() {
    if (InfinityDimension(9).isUnlocked)
  ...
  tryAutoUnlock() {
    if (!EternityMilestone.autoUnlockID.isReached || InfinityDimension(9).isUnlocked) return;

...
export function getDimensionFinalMultiplierUncached(tier) {
  ...
  if (NormalChallenge(11).isRunning && tier > 7) return DC.D1;
...
function applyNDMultipliers(mult, tier) {
  ...
  multiplier = multiplier.timesEffectsOf(
    tier === 9 ? Achievement(23) : null,
    tier < 9 ? Achievement(34) : null,
    tier <= 4 ? Achievement(64) : null,
    tier < 9 ? TimeStudy(71) : null,
    tier === 9 ? TimeStudy(214) : null,
    tier > 1 && tier < 9 ? InfinityChallenge(8).reward : null
  );
...
export function buyOneDimension(tier) {
  ...
  if (tier === 9 && Enslaved.isRunning && AntimatterDimension(9).bought >= 1) return false;
...
export function buyManyDimension(tier) {
  ...
  if (tier === 9 && Enslaved.isRunning) return buyOneDimension(9);
...
class AntimatterDimensionState extends DimensionState {
  constructor(tier) {
    ...
    const BASE_COSTS = [null, 10, 100, 1e4, 1e6, 1e9, 1e13, 1e18, 1e24, 1e31];
    ...
    const C6_BASE_COSTS = [null, 10, 100, 100, 500, 2500, 2e4, 2e5, 4e6, 4e7];
    this._c6BaseCost = C6_BASE_COSTS[tier];
    const C6_BASE_COST_MULTIPLIERS = [null, 1e3, 5e3, 1e4, 1.2e4, 1.8e4, 2.6e4, 3.2e4, 4.2e4, 5.4e4];
  ...
  get rateOfChange() {
    ...
    } else if (NormalChallenge(13).isRunning) {
  ...
  get continuumValue() {
    ...
    // Nameless limits dim 9 purchases to 1 only
    // Continuum should be no different
    if (this.tier === 9 && Enslaved.isRunning) return 1;
  ...
  get isAvailableForPurchase() {
    ...
    return this.tier < 8 || !NormalChallenge(11).isRunning;
  }
  ...
  challengeCostBump() {
    ...
    else if (NormalChallenge(10).isRunning) this.multiplySameCosts();
  }
  ...
  get productionPerSecond() {
    ...
    if (NormalChallenge(13).isRunning) {
      if (tier === 2) amount = amount.pow(1.7);
      if (tier === 4) amount = amount.pow(1.5);
      if (tier === 6) amount = amount.pow(1.3);
      if (tier === 8) amount = amount.pow(1.1);
    }
...
export const AntimatterDimensions = {
  ...
  get buyTenMultiplier() {
    ...
    if (NormalChallenge(9).isRunning) return mult.times(3);
    return mult;
  },
  ...
  tick(diff) {
    ...
    let maxTierProduced = EternityChallenge(3).isRunning ? 3 : 8;

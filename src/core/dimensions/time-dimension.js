...
export function maxAllTimeDimensions() {
  ...
  for (let i = 9; i > 0 && TimeDimension(i).bought === 0; i--) {
...
class TimeDimensionState extends DimensionState {
  constructor(tier) {
    ...
    const BASE_COSTS = [null, DC.D1, DC.D5, DC.E2, DC.E3, DC.E6, DC.E2350, DC.E2650, DC.E3000, DC.E3350];
    this._baseCost = BASE_COSTS[tier];
    const COST_MULTS = [null, 3, 9, 27, 81, 243, 72900, 218700, 656100, 1968300];
    this._costMultiplier = COST_MULTS[tier];
    const E6000_SCALING_AMOUNTS = [null, 7322, 4627, 3382, 2665, 2198, 833, 689, 562, 456];
  ...
  nextCost(bought) {
    if (this._tier > 5 && bought < this.e6000ScalingAmount) {
    ...
    if (PelleRifts.paradox.milestones[0].canBeApplied && this._tier > 5) {
      return cost.div("1e2250").pow(0.5);
    }
    return cost;
  }
  ...
  get isUnlocked() {
    return this._tier < 6 || TimeStudy.timeDimension(this._tier).isBought;
  }
  ...
  get multiplier() {
    ...
    const dim = TimeDimension(tier);
    const bought = tier === 9 ? Math.clampMax(dim.bought, 1e8) : dim.bought;
  ...
  get powerMultiplier() {
    return DC.D4
      .timesEffectsOf(this._tier === 9 ? GlyphSacrifice.time : null)
...
export const TimeDimensions = {
  ...
  tick(diff) {
    ...
    if (EternityChallenge(7).isRunning) {
      TimeDimension(1).produceDimensions(InfinityDimension(9), diff);
    } else {
      TimeDimension(1).produceCurrency(Currency.timeShards, diff);
    }

    EternityChallenge(7).reward.applyEffect(production => {
      InfinityDimension(9).amount = InfinityDimension(9).amount.plus(production.times(diff / 1000));
    });
  }
};

export function tryUnlockTimeDimensions() {
  if (TimeDimension(9).isUnlocked) return;

...
export const AD = {
  total: {
    ...
    displayOverride: dim => {
      if (dim) {
        const singleMult = NormalChallenge(13).isRunning
      ...
      if (NormalChallenge(13).isRunning) return `${format(MultiplierTabHelper.actualNC12Production(), 2)}/sec`;
  ...
  effectNC: {
    ...
    isActive: () => [2, 3, 13].some(c => NormalChallenge(c).isRunning),
    icon: MultiplierTabIcons.CHALLENGE("infinity"),
  },
  nerfIC: {

...
export class DimBoost {
  static get power() {
    if (NormalChallenge(8).isRunning || NormalChallenge(9).isRunning) {
      return DC.D1;
    }
  ...
  static get maxDimensionsUnlockable() {
    return NormalChallenge(11).isRunning ? 7 : 9;
  }
  ...
  static bulkRequirement(bulk) {
    ...
    if (tier === 7 && NormalChallenge(11).isRunning) {
      amount += Math.round((targetResets - 4) * (20 - discount));
    } else if (tier === 9) {
      amount += Math.round((targetResets - 6) * (15 - discount));
    }
  ...
  static get unlockedByBoost() {
    ...
    } else if (boosts === 5 && !NormalChallenge(11).isRunning && !EternityChallenge(3).isRunning) {
    ...
    if (boosts > 0) dimensionRange = `to Dimensions 1-${Math.min(boosts + 1, 9)}`;

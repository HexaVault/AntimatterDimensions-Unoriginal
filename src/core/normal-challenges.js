...
export function updateNormalAndInfinityChallenges(diff) {
  if (NormalChallenge(12).isRunning || InfinityChallenge(6).isRunning) {
    ...
    if (Currency.matter.gt(Currency.antimatter.value) && NormalChallenge(12).isRunning && !Player.canCrunch) {
...
class NormalChallengeState extends GameMechanicState {
  ...
  get isRunning() {
    const isPartOfIC1 = this.id !== 10 && this.id !== 13 && this.id !== 14 && this.id !== 15;
  ...
  start() {
    ...
    if (Enslaved.isRunning && EternityChallenge(6).isRunning && this.id === 11) {

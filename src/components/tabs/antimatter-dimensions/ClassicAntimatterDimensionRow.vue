...
export default {
  ...
  computed: {
    ...
    amountText() {
      ...
      const amount = this.tier < 9 ? format(this.amount, 2) : formatInt(this.amount);
    ...
    boughtTooltip() {
      if (this.isCapped) return `Nameless prevents the purchase of more than ${format(1)} 9th Antimatter Dimension`;
  ...
  methods: {
    update() {
      ...
      if (tier === 9 && this.isDoomed) this.formattedAmount = formatInt(this.amount);
